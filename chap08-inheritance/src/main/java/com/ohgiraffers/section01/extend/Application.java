...
public class Application {
     ...
     * 멤버를 자식이 물려 받아 자기의 멤버인 것 처럼 사용할 수 있도록 만든 기술이다.
     *
     ...
     *  1-2. 공통적으로 사용하는 코드가 부모 클래스에서만 존재하면 수정사항이 생길 시 부모만 수정해도 전체적으로 적용된다. (유지보수성 증가)
     * 2. 클래스 간의 계층 관계가 형성되며 다형성 문법적인 토대가 된다.
     *
     * 하지만 단점도 존재한다.
     * 1. 부모 클래스의 기능을 추가/변경할 시 자식클래스가 정상적으로 동작하는지
     * 예측이 힘들다. 상속 구조가 복잡해 질 수록 그 영향에 대한 예측이 힘들어 이러한 단점이 유지보수성이 증가하는 장점과는 반대로 유지보수에 악영향을 끼친다.
     * 2. 또한 부모클래스의 변경도 쉽지 않다. 자식 클래스에서 중요하게 사용하는 기능인 경우 부모 클래스를 변경할 시 자식 클래스에 모두 영향을 줄 수 있다.
     * 역시 유지보수성에 악 영향을 미친다.
     ...
     * */
        ...
        Car car = new Car();    //cf) 부모클래스를 자식클래스로 대체하여도 에러가 뜨지않아야 한다!!ex)자식의 자료형을 부모자료형에 넣어도 가능. 즉, 상속은 자식은 부모껄 써도 부모는 자식껄 접근못한다.
                                   // 부모 = 악기 자식 = 악기의 종류들 ex) 관악기, 현악기...
        car.soundHorn();
        car.run();
        car.soundHorn();
        car.stop();
        ...
        fireCar.toString();   //object에 존재하는 메소드
        fireCar.run();
        fireCar.soundHorn();
        // 상속은 확장이다.
        fireCar.sprayWater();
        ...
        // FireCar fireCar1 = racingCar; 자식은 자식한테 넣어줄 수 없다. 또한 부모걸 자식한테 넣어줄 수없다. //넣어주려면 형변환이 필요하다. chater9에서 배운다.
    }
}
