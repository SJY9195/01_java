...
public class Car {
    ...
    private boolean isOn;   ///boolean의 기본값은 fasle 이다.

    private int speed;

    // 시동 걸기 - 시동이 걸려있는 상태의 경우는 할 일이 없고,
    ...
    public void startUp() {
        if (isOn) {
            ...
            this.isOn = true;
            System.out.println("시동이 걸렸습니다.");    //한번 호출하면 기본값인 false라 else가 되지만 그 이후에는 isOn이 true이므로 시동이 걸려있는 상태가 된다.
        }
    }

    // 시동이 걸린 상태인 경우 호출 마다 속도를 10km/h 씩 증가시킨다.
    ...
    public void stop() {
        if (isOn) {
            if (this.speed > 0) {
                while (this.speed > 0) {
                    --this.speed; // 전위 감소 연산자사용 // this.speed = this.speed - 10; 이건 -10씩 감소
            ...
            System.out.println("시동을 걸지 않은 경우 유압으로 인해" +
                    " 브레이크가 점점 무거워 집니다.");
        }
    }
