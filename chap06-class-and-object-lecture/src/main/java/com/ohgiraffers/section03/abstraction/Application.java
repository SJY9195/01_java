...
public class Application {
    public static void main(String[] args) {
        ...
        // 객체지향 프로그래밍에 대해 이해하고 객체와 클래스를
        // 설계하여 프로그래밍 할 수 있다.

        /*
        * 객체 지향 프로그래밍이란?
        * 현실세계에 모든 사건(event) 는 개체와 객체의 상호작용에 의해
        * 일어난다는 세계관을 프로그램을 만들 때 이용하여 새로운 세계를 창조하는 방법론이다.
        * 현실 세계와 닮아있지만, 다른 점은 객체간의 상호작용을 한다는
        * 사실은 현실 세계와 동일하지만 의인화 기법이 적용되어 현실세계에서
        * 불가능한 무생물이나 개념 같은 존재들도 하나의 주체로
        * 본인의 상태를 스스로 제어하고 행동하도록 한다는 것이 다른점이다.
        * 모든 객체는 책임을 가지고 자신의 역할을 수행해야 한다.
        * 또한 하나의 객체는 너무 많은 일을 수행하는 것이 아닌 적절한
        * 책임을 갖고 있어야 한다. (단일 책임의 원칙)
        * */

        /*
        * 객체를 설계하기 위해서는 복잡한 현실세계를 그대로 반영하기에는
        * 너무 방대하고 복잡하기 때문에 현실 세계를 프로그램의 목적에 맞게
        * 단순화하는 추상화 라는 기법을 적용하게 된다.
        *
        * 추상화란?
        * 공통된 부분을 추출하고 공통되지 않은 부분을 제거한다는 의미를 가지는
        ...
        * 순차적으로 기술한 것을 메소드라고 한다.
        * 상호작용에 중요한 것은 메세지를 송수신하고 그 방법을 기술하여 메세지에 응답하는 상호작용을 설계하는 것이다.
        ...
        * */
        ...
        //절차지향 객체지향과의 차이를 알아두는게 좋다!! (면접질문가능성있음)
        ...
        *
        * 시스템 요구사항.
        * 1. 카레이서는 시동 걸기, 엑셀 밟기, 브레이크 밟기, 시동 끄기를 할
        * 2. 자동차는 시동 걸기, 앞으로 가기, 멈추기, 시동 끄기
        * 3. 자동차는 처음에 멈춘 상태(시동이 꺼진 상태)로 대기한다.
        * 4. 카레이서는 먼저 자동차에 시동을 건다. 이미 걸려있는 경우는 다시 걸 수 없다.
        * 5. 카레이서가 엑셀을 밟으면 시동이 걸린 상태일 경우 10km/h 씩 증가하며 앞으로 나간다.
        * 6. 자동차가 달리는 중인 경우 브레이크를 밟으면 자동차의 시속은 0으로 떨어지며 멈춘다.
        * 7. 브레이크를 밝을 때 자동차가 달리는 중이 아니라면 이미 멈춰있는 상태라고 안내한다.
        ...
        * */
        ...
        while(true){
            ...
            switch(no){     //Application 사용자고 사용자가 CarRacer객체한테 명령하면 CarRacer가 Car한테 명령하는 방식!!   //역할이 분리되어있는것이다!! Application(사용자)역할, Car역할, CarRacer역할
                ...
                case 9 :
                    System.out.println("프로그램을 종료합니다."); return;   //메소드 종료
                default :
